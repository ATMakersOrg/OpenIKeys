...
IKString::IKString(const TCHAR *psz)
{
#if IKSTRING_DYNAMIC
	m_data = 0; m_size = 0;
#endif
...
IKString::IKString(BYTE *data, int len)
{
	...
	//  assume it's single-byte
	//int i = IKString::strlen((char *)data);
	int i = len;
	
#ifndef IKSTRING_DYNAMIC
	if (i>MAX_STRING-1)
		i = MAX_STRING-1;
#endif

	if(i>0)
		IKString::strncpy(m_data,(char *)data,i);
...
const IKString& IKString::operator+=(const TCHAR * lpsz)
{
...
int IKString::Find ( const TCHAR * lpsz )
{
...
void IKString::TrimRight()
{
	...
	int i = GetLength();
    if (i > 0) {
        while ( (m_data[i-1]==CHAR(' ') || m_data[i-1]==CHAR('\t')) && i>0)
            i--;
        result = Left (i);
    }
...
void IKString::TrimLeft()
{
    ...
    if (GetLength() > 0) {
        while ( (m_data[i]==CHAR(' ') || m_data[i]==CHAR('\t')) && i<GetLength())
            i++;
