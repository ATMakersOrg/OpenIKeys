...
{
	//  be careful in here to only queue up the data
	...
	DebugLogToFile("IKDevice.OnDataReceived [%d][%s] InputQSize=%d", data[0], (LPSTR)GetCommandName(data[0]), m_inputQueue.GetSize());
}
...
void IKDevice::PostCommand(BYTE *command)
{
	...
	//if (command[0] != IK_CMD_LED)
	//{
	//	DebugLogToFile("IKDevice.PostCommand [%d][%s] CmdQSize=%d", command[0], (LPSTR)GetCommandName(command[0]), m_commandQueue.GetSize());
	//}
}
...
void IKDevice::SendCommand(BYTE *command)
{
	...
	if (m_writeProc!=NULL)
	{
		...
		DebugLogToFile("IKDevice.SendCommand - Device Write Proc(idx=%d, cmd=([%d][%s])[%d][%d][%d][%d])", m_index, command[0], (LPSTR)GetCommandName(command[0]), command[1], command[2], command[3], command[4]);
	}
}
...
void IKDevice::Periodic()
{
	...
	// JR - Jun 2012 - send the Get Version command
	if (m_firmwareVersionMajor == 0) 
	{
		BYTE command[IK_REPORT_LEN] = {IK_CMD_GET_VERSION,0,0,0,0,0,0,0};
		PostCommand(command);
	}
	else 
	{
...
void IKDevice::Start()
{
	...
	// JR - Jun 2012 - ask the device for its firmware version
...
void IKDevice::ProcessCommands()
{
	...
	while (!m_commandQueue.isEmpty())
	{
		...
		//if (qe.buffer[0] != IK_CMD_LED) 
		//{
		//	DebugLogToFile("IKDevice.ProcessCommands [%d, %d, %d, %d, %d][%s] CmdQSize=%d", qe.buffer[0], qe.buffer[1], qe.buffer[2], qe.buffer[3], qe.buffer[4], (LPSTR)GetCommandName(qe.buffer[0]), m_commandQueue.GetSize());
		//}
...
void IKDevice::ProcessInput()
{
	while (!m_inputQueue.isEmpty())
	{
		...
		DebugLogToFile("IKDevice.ProcessInput [cmd=%d, x=%d, y=%d, %d][%s] InputQSize=%d", qe.buffer[0], qe.buffer[1], qe.buffer[2], qe.buffer[3], (LPSTR)GetCommandName(qe.buffer[0]), m_inputQueue.GetSize());
		...
		case IK_EVENT_VERSION:
			...
			if (m_firmwareVersionMajor == 0)
			{
				m_firmwareVersionMajor = qe.buffer[1];
				m_firmwareVersionMinor = qe.buffer[2];
			}
			break;
...
{			
	//DebugLogToFile("IKDevice::Interpret start");
...
{
	DebugLogToFile("IKDevice.DoKey - code=%d[0x%0x] drctn=%d[%s]", code, code, direction, ((direction == IK_DOWN) ? "Down" : ((direction == IK_UP) ? "Up" : "Toggle")));
	...
	for (int i = 0; i < n; i++)
	{
		if (universalCode == UniversalToHid[i].universalCode)
		{
			...
			*usageID   = UniversalToHid[i].HIDUsageID;
			...
			DebugLogToFile("IKDevice.UniversalToHID [Code=%d[0x%0x], UsagePage=%d[0x%0x], UsageID=%d[0x%0x]]", universalCode, universalCode, *usagePage, *usagePage, *usageID, *usageID);
...
IKString IKDevice::FirmwareVersion()
{
	IKString firmwareVersion = TEXT("xxx.xxx");
	firmwareVersion.Format("%d.%d", m_firmwareVersionMajor, m_firmwareVersionMinor);
...
int IKDevice::ExecuteUniversalData(BYTE *theRealData, bool bParForceKeyUp /* =false */)
{
	...
	BYTE *pTheData = theRealData;
	if (pTheData[0] == UNIVERSAL_SEPARATE_REPEAT_KEY)
	{
		pTheData = m_lastExecuted;
	}
	else
	{
		m_lastExecuted = pTheData;
	}
	if (pTheData == NULL)
		return 0;
	if (pTheData[0] == 0)
	...
	{
		//  single mouse movement, do acceleration
		dataSendDelay = 0;
...
//---
		...
		default:
			...
			else
			{
				if ((codeCount > 1) || bForceKeyUp)
				{
					...
					//  do it all now.
					int keyDownDelay = DATAI(TEXT("Key Down Time"), 5);
		...
		if (IKSettings::GetSettings()->m_iShiftKeyAction == kSettingsShiftLatching)
		{
			if (!IsModifier(myData[j]) && !IsMouseAction(myData[j]) && (myData[j] != UNIVERSAL_NON_REPEATING))
			{
				...
				{
					ndown--;
					PostDelay( DATAI(TEXT("Key Down Time"), 5));
...
IKString IKDevice::GetCommandName ( BYTE command )
{
	...
	case IK_CMD_CP_REPORT_REALTIME:
		s = TEXT("IK_CMD_CP_REPORT_REALTIME");
		break;
	...
	case IK_EVENT_MEMBRANE_PRESS:
		s = TEXT("IK_EVENT_MEMBRANE_PRESS");
		break;

	case IK_EVENT_MEMBRANE_RELEASE:
		s = TEXT("IK_EVENT_MEMBRANE_RELEASE");
		break;

	case IK_EVENT_SWITCH:
		s = TEXT("IK_EVENT_SWITCH");
		break;

	case IK_EVENT_SENSOR_CHANGE:
		s = TEXT("IK_EVENT_SENSOR_CHANGE");
		break;

	case IK_EVENT_VERSION:
		s = TEXT("IK_EVENT_VERSION");
		break;

	case IK_EVENT_EEPROM_READ:
		s = TEXT("IK_EVENT_EEPROM_READ");
		break;

	case IK_EVENT_ONOFFSWITCH:
		s = TEXT("IK_EVENT_ONOFFSWITCH");
		break;

	case IK_EVENT_NOMOREEVENTS:
		s = TEXT("IK_EVENT_NOMOREEVENTS");
		break;

	case IK_EVENT_MEMBRANE_REPEAT:
		s = TEXT("IK_EVENT_MEMBRANE_REPEAT");
		break;

	case IK_EVENT_SWITCH_REPEAT:
		s = TEXT("IK_EVENT_SWITCH_REPEAT");
		break;

	case IK_EVENT_CORRECT_MEMBRANE:
		s = TEXT("IK_EVENT_CORRECT_MEMBRANE");
		break;

	case IK_EVENT_CORRECT_SWITCH:
		s = TEXT("IK_EVENT_CORRECT_SWITCH");
		break;

	case IK_EVENT_CORRECT_DONE:
		s = TEXT("IK_EVENT_CORRECT_DONE");
		break;

	case IK_EVENT_EEPROM_READBYTE:
		s = TEXT("IK_EVENT_EEPROM_READBYTE");
		break;

	case IK_EVENT_DEVICEREADY:
		s = TEXT("IK_EVENT_DEVICEREADY");
		break;

	case IK_EVENT_AUTOPILOT_STATE:
		s = TEXT("IK_EVENT_AUTOPILOT_STATE");
		break;

	case IK_EVENT_DELAY:
		s = TEXT("IK_EVENT_DELAY");
		break;

	case IK_EVENT_ALL_SENSORS:
		s = TEXT("IK_EVENT_ALL_SENSORS");
		break;
...
{
	DebugLogToFile("IKDevice::InterpretRaw start");
	//  don't bother if we're not connected and switched on
	if (!IsOpen())
		return;
	if (!IsSwitchedOn())
...
bool IKDevice::GetRawEvent(BYTE *bytes)
{
	...
	DebugLogToFile("IKDevice::GetRawEvent - [%d, %d, %d, %d][%s] RawQSize=%d", qe.buffer[0], qe.buffer[1], qe.buffer[2], qe.buffer[2], (LPSTR)GetCommandName(qe.buffer[0]), m_rawQueue.GetSize());
