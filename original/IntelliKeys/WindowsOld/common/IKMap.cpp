...
{
	//DebugLogToFile("IKMap::Add start. Key=[%s] Value=[%s]", (TCHAR*)(key), (TCHAR*)(value));
	...
	if (i >= 0)
	{
		...
		//DebugLogToFile("IKMap::Add - Found Key: [%s] Index: [%d] CurrentValue: [%s]", (TCHAR*)key, i, (TCHAR*)(m_pairs[i].value));

		m_pairs[i].value = value;

		//DebugLogToFile("IKMap::Add end. Reset Index=[%d] [%s]=[%s]", i, (TCHAR*)m_pairs[i].key, (TCHAR*)(m_pairs[i].value));
	...
	//DebugLogToFile("IKMap::Add end. Added Index=[%d] [%s]=[%s]", m_nPairs - 1, (TCHAR*)(m_pairs[m_nPairs - 1].key), (TCHAR*)(m_pairs[m_nPairs - 1].value));
...
bool IKMap::Remove(IKString key)
{
	...
	if (i >= 0)
	{
		...
		//DebugLogToFile("IKMap::Remove - Found Key - Index=[%d] [%s]=[%s]", i,  (TCHAR*)(m_pairs[m_nPairs - 1].key), (TCHAR*)(m_pairs[m_nPairs - 1].value));
...
{	
	...
	//  open the file
	IKFile inputFile;
	bool bOpened = inputFile.Open ( filename, IKFile::modeRead);
	if (!bOpened)
	{
		return false;
	}
	//  get file length
	int nBytes = inputFile.GetLength();
	//  determine if file is Unicode
	int nbrBytesPerChar = 1;
	if (inputFile.IsUnicode())
	{
		nbrBytesPerChar = 2;
		//  skip first two bytes
		inputFile.Seek(2, IKFile::begin);
		nBytes -= 2;
	}
	else if (inputFile.IsUtf8())
	{
		nbrBytesPerChar = 1;
		//  skip first three bytes
		inputFile.Seek(3, IKFile::begin);
		nBytes -= 3;
	}

	//  read the contents.
	//  add a line feed at the end.
	int nChars = (nBytes / nbrBytesPerChar) + 1;
	TCHAR *chars = new TCHAR[nChars + 1];
	int nread = inputFile.Read((void *)chars, nBytes);
	inputFile.Close();
	//  add a line ending
	chars[nChars - 1] = TEXT('\n');
	chars[nChars] = 0;
		...
		case TEXT('\n'):
			if (!strLine.IsEmpty())
			{
				...
				if (!strLine.IsEmpty())
				{
					...
					IKString value;
					i=0;
					while ((i < strLine.GetLength()) && (strLine.GetAt(i) != TEXT(' ')) && (strLine.GetAt(i) != TEXT('\t')))
					{
						i++;
					}
					key = strLine.Left(i);
					...
					if (!key.IsEmpty())
					{
						...
						//DebugLogToFile("IKMap::Read - Input Line: [%s] => Key: [%s] Value: [%s]", (TCHAR*)strLine, (TCHAR*)key, (value.IsEmpty()) ? TEXT("[EmptyString]") : (TCHAR*)value);
...
bool IKMap::Write(IKString filename)
{
	...
	//  create the file
	IKFile outputFile;
	bool bOpened = outputFile.Open(filename, IKFile::modeWrite | IKFile::modeCreate);
	if (!bOpened)
	{
		return false;
	}
	...
	{
		outputFile.MarkAsUTF8();
	}
	else
	{
		outputFile.MarkAsUnicode();
	}

	//DebugLogToFile("IKMap::Write - File: [%s]", (TCHAR*)filename);
	...
	for (int i = 0; i < m_nPairs; i++)
	{
		...
		//DebugLogToFile("IKMap::Write - Index=[%d] [%s]=[%s]", i, (TCHAR*)(m_pairs[i].key), (TCHAR*)(m_pairs[i].value));
	}
