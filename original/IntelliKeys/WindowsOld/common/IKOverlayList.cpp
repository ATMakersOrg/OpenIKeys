...
{
  // (n > GetCount() ? return : m_selected = n);
	if (n > GetCount())
	{
		return;
	}
...
{
  // return (n > GetCount() ? IKString(TEXT("")) : m_entries[n-1]);
	if (n > GetCount())
	{
		return IKString(TEXT(""));
	}
...
IKString IKOverlayList::GetEncodedList()
{
  ...
  // IKString r = IKUtil::IntToString(m_selected) + "|";
...
void IKOverlayList::AddEntry(IKString entry, bool bSelected /*=false*/)
{
  ...
  //  if ( bSelected ) { SetNumSelected(m_count); }
  // }
	int i = Find(entry);
	if (i == -1)
	{
