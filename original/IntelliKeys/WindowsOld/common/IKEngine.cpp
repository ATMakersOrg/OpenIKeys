...
void IKEngine::Periodic()
{
	...
	if (once==0)
	{
		...
		extPeriod2		= DATAI(TEXT("External_Command_Period 2"),250);
...
void IKEngine::ProcessExternalCommands()
{
	...
	//DebugLogToFile("IKEngine::ProcessExternalCommands: start");
	...
	{
		//DebugLogToFile("IKEngine::ProcessExternalCommands: end 1");
		return;
	}
	...
	case kQuerySendOverlayWithName:
		{
			...
			//DebugLogToFile("IKEngine::ProcessExternalCommands: - 1 - Command: [%d][%s] Sender: [%s] File: [%s]  Name: [%s]", kQuerySendOverlayWithName, (LPSTR)IKMessage::GetMessageName(kQuerySendOverlayWithName), (LPSTR)sender, (LPSTR)file, (LPSTR)name);
		...
		//DebugLogToFile("IKEngine::ProcessExternalCommands: - 2 - Command: [%d][%s] - call IKMessage::Respond(%s, %d [%s])", kQuerySendOverlayWithName, (LPSTR)IKMessage::GetMessageName(kQuerySendOverlayWithName), channel, kResponseNoError, (LPSTR)IKMessage::GetMessageName(kResponseNoError));
		...
		IKMessage::Respond(TEXT(channel), kResponseNoError, 0, 0);
		{
			...
			//DebugLogToFile("IKEngine::ProcessExternalCommands: - 3 - Command: [%d][%s] - call IKMessage::Send(%s, %d [%s])", kQuerySendOverlayWithName, (LPSTR)IKMessage::GetMessageName(kQuerySendOverlayWithName), "control channel", kQueryGlobalData, (LPSTR)IKMessage::GetMessageName(kQueryGlobalData));

			IKMessage::Send(TEXT("control panel"), kQueryGlobalData, data, ndata, 0, 0);
		}

		//DebugLogToFile("IKEngine::ProcessExternalCommands: - 4 - Command: [%d][%s]", kQuerySendOverlayWithName, (LPSTR)IKMessage::GetMessageName(kQuerySendOverlayWithName));
	...
	case kQuerySoftwareVersion:
		{
			...
			IKString version = VER_STRING_PRODUCTVERSION;
			BYTE data[255];
	...
	//DebugLogToFile("IKEngine::ProcessExternalCommands: end 2");
}
...
{
	DebugLogToFile("IKEngine::OnNewCustomOverlayFile: File: [%s]", (LPSTR)filename);
	...
	if(bCopied)
	{
		...
		DebugLogToFile("IKEngine::OnNewCustomOverlayFile: Copied New File: [%s]", (LPSTR)newName);
	...
	{
		DebugLogToFile("IKEngine::OnNewCustomOverlayFile: Copy Failed - New File: [%s]", (LPSTR)newName);
	}
}
...
void IKEngine::MakeGlobalData ( BYTE *data, int *datalen )
{
	...
	string = VER_STRING_PRODUCTVERSION;
...
void IKEngine::OnNewCustomOverlaySettings(BYTE *data, int datalen)
{
	...
	{
		DebugLogToFile("IKEngine::OnNewCustomOverlaySettings: Data[%d]=0x%X", idx, data[idx]);
	}
	...
	DebugLogToFile("IKEngine::OnNewCustomOverlaySettings: Loaded Temp Custom Overlay File: [%s]", (LPSTR)newName);
...
void IKEngine::ReloadCustomOverlayFile(IKString filename)
{
	...
	if (bLoaded)
	{
		DebugLogToFile("IKEngine::ReloadCustomOverlayFile: Reloaded Overlay File: [%s]", (LPSTR)filename);
	...
	{
		DebugLogToFile("IKEngine::ReloadCustomOverlayFile: Reload Failed - Overlay File: [%s]", (LPSTR)filename);
...
void IKEngine::ProcessExternalCommands2 ()
{
	...
	//DebugLogToFile("IKEngine::ProcessExternalCommands2: start");

	response = IKMsg::Receive ( TEXT(channel), &command, (void *)data, 2000, &datalen );
	if (response == kResponseNoCommand)
	{
		//DebugLogToFile("IKEngine::ProcessExternalCommands2: end 1");
		return;
	}
	...
	//DebugLogToFile("IKEngine::ProcessExternalCommands2: end 2");
}
